...
from swift.common.exceptions import DevIdBytesTooSmall, RingLoadError
...
RING_CODECS = {
    0: {
        ...
        "deserialize": lambda cls, reader, metadata_only: pickle.load(reader),
    },
    1: {
...
class RingData(object):
    ...
    def set_dev_id_bytes(self, new_dev_id_bytes):
        """
        Resize the replica2part2dev_id table to use a new ``dev_id_bytes``

        :raises ValueError: if ``new_dev_id_bytes`` is not allowed
        :raises DevIdBytesTooSmall: if ``new_dev_id_bytes`` is too small to
                                    accomodate all dev ids in use
        """
        if new_dev_id_bytes not in BYTES_TO_TYPE_CODE:
            raise ValueError('dev_id_bytes must be one of %r'
                             % (tuple(BYTES_TO_TYPE_CODE),))

        if new_dev_id_bytes == self.dev_id_bytes:
            return  # Already done!
        min_dev_id_bytes = calc_dev_id_bytes(self.max_dev_id)
        if min_dev_id_bytes > new_dev_id_bytes:
            raise DevIdBytesTooSmall('Too many devices for %d-byte '
                                     'device ids' % new_dev_id_bytes)
    ...
    def max_dev_id(self):
        """
        The highest dev id that's been claimed.

        Note that this may be larger than what's actually used in the
        replica2part2dev_id table, but it can't be lower.
        """
        if not self.devs:
            return 0

        for i, dev in enumerate(reversed(self.devs)):
            if dev is not None:
                break
        return len(self.devs) - 1 - i
    ...
    def load(cls, filename, metadata_only=False):
        ...
        """
        with RingReader(filename) as reader:
    ...
    def serialize_v1(self, writer):
        ...
        for part2dev_id in ring['replica2part2dev_id']:
            if six.PY2:
                ...
                writer.write(part2dev_id.tostring())
            else:
    ...
    def save(self, filename, mtime=1300507380.0,
        ...
        with RingWriter(filename, mtime) as writer:
